#!/bin/bash
echo "=== Ampliando columna symbol ==="
docker exec trading-bot-platform-postgres-1 psql -U admin -d tradingbot -c "ALTER TABLE bot_configs ALTER COLUMN symbol TYPE VARCHAR(50);"
docker exec trading-bot-platform-postgres-1 psql -U admin -d tradingbot -c "ALTER TABLE positions ALTER COLUMN symbol TYPE VARCHAR(50);"

echo "=== Limpiando y restaurando datos ==="
...
docker exec trading-bot-platform-postgres-1 psql -U admin -d tradingbot -c "SELECT COUNT(*) as posiciones FROM positions;"
