...
# -*- coding: utf-8 -*-
"""
DYNAMIC SWING VWAP v4.2 - Trend Score + Estructura de Mercado Confirmada
"""
import pandas as pd
import numpy as np
...
class VWAPConfig:
    ...
    # NUEVOS PARAMETROS v4.2
    trend_ema_fast: int = 50      # EMA rapida para filtro
    trend_ema_slow: int = 200     # EMA lenta para confirmacion de tendencia
    min_atr_dist_to_vwap: float = 0.3
    sl_atr_mult: float = 2.5      # AUMENTADO: stops mas amplios
    tp_atr_mult: float = 4.0      # AUMENTADO: targets mas amplios
    trend_score_threshold: int = 2 # Requiere +/-2 para cambiar tendencia
    max_bars_no_swing: int = 30   # Reset a neutral si no hay swings
...
class DynamicSwingVWAP:
    ...
    def _reset_state(self):
        ...
        self.phL = np.nan
        self.plL = np.nan
        # v4.2: trend_score con memoria (-5 a +5)
        self.trend_score = 0
        self.trend_dir = 0
        self.last_swing_label_high = ""
        self.last_swing_label_low = ""
    ...
    def _update_trend_score(self, swing_label: str):
        """v4.2: Score con memoria. Requiere +/-threshold para cambiar tendencia."""
        threshold = self.config.trend_score_threshold

        if swing_label in ["HH", "HL"]:
            self.trend_score = min(self.trend_score + 1, 5)
        elif swing_label in ["LH", "LL"]:
            self.trend_score = max(self.trend_score - 1, -5)
        elif swing_label in ["IH", "IL"]:
            # Swings iniciales: movimiento pequeno
            if "H" in swing_label:
                self.trend_score = min(self.trend_score + 1, 5)
            else:
                self.trend_score = max(self.trend_score - 1, -5)
        ...
        # Aplicar threshold
        if self.trend_score >= threshold:
            self.trend_dir = 1
        elif self.trend_score <= -threshold:
            self.trend_dir = -1
        # Si esta entre -threshold+1 y threshold-1, mantiene direccion anterior
        # (histeresis natural)
    ...
    def calculate(self) -> pd.DataFrame:
        ...
        trend_score_arr = np.full(self.n, 0)
        ...
        for i in range(self.n):
            ...
            if is_swing_high:
                ...
                self.last_swing_label_high = swing_label
                self._update_trend_score(swing_label)
            ...
            if is_swing_low:
                ...
                self.prev_low = low
                self.last_swing_str = swing_label
                self.last_swing_label_low = swing_label
                self._update_trend_score(swing_label)

            # v4.2: Reset a neutral si no hay swings recientes
            last_swing_bar = max(
                self.phL if not np.isnan(self.phL) else -9999,
                self.plL if not np.isnan(self.plL) else -9999
            )
            bars_since_swing = i - last_swing_bar
            if bars_since_swing > self.config.max_bars_no_swing:
                # Decaimiento gradual del score
                if self.trend_score > 0:
                    self.trend_score = max(self.trend_score - 1, 0)
                elif self.trend_score < 0:
                    self.trend_score = min(self.trend_score + 1, 0)
                # Solo resetear trend_dir si score llega a 0
                if self.trend_score == 0:
                    self.trend_dir = 0
            ...
            trend_dir_arr[i] = self.trend_dir
            swing_label_arr[i] = self.last_swing_str
            trend_score_arr[i] = self.trend_score
...
def strategy(df, params):
    config = VWAPConfig(
        ...
        trend_ema_fast=params.get("trend_ema_fast", 50),
        trend_ema_slow=params.get("trend_ema_slow", 200),
        ...
        trend_score_threshold=params.get("trend_score_threshold", 2),
        max_bars_no_swing=params.get("max_bars_no_swing", 30),
    )

    ema_fast = df['close'].ewm(span=config.trend_ema_fast, adjust=False).mean()
    ema_slow = df['close'].ewm(span=config.trend_ema_slow, adjust=False).mean()
    ...
    for i in range(len(df)):
        ...
        ema_f = ema_fast.iloc[i]
        ema_s = ema_slow.iloc[i]

        if result['buy_signal'].iloc[i]:
            # v4.2: LONG solo si:
            # 1. trend_dir == 1 (score confirmado)
            # 2. precio > EMA50 (tendencia alcista a corto plazo)
            # 3. precio > EMA200 (tendencia alcista a largo plazo)
            if close > ema_f and close > ema_s:
                signal.iloc[i] = 1
                atr_val = result['atr'].iloc[i]
                if not np.isnan(atr_val):
                    stop_loss.iloc[i] = close - atr_val * config.sl_atr_mult
                    take_profit.iloc[i] = close + atr_val * config.tp_atr_mult

        elif result['sell_signal'].iloc[i]:
            # v4.2: SHORT solo si:
            # 1. trend_dir == -1 (score confirmado)
            # 2. precio < EMA50 (tendencia bajista a corto plazo)
            # 3. precio < EMA200 (tendencia bajista a largo plazo)
            if close < ema_f and close < ema_s:
                signal.iloc[i] = -1
                atr_val = result['atr'].iloc[i]
                if not np.isnan(atr_val):
