...
def _extract_signal_tp_levels(sig: AISignal, fill_price: float | None = None) -> list[dict]:
    """Extract TP levels from signal features generated by dynamic_risk_builder.

    If the signal has a `tp_levels` list, use it directly.  Otherwise fall back
    to legacy signal.take_profit_1/2.  Prices are optionally re-based to the
    actual fill price while preserving the original R-multiples.
    """
    feats = sig.features or {}
    tp_levels = feats.get("tp_levels") or []
    if not tp_levels and sig.take_profit_1:
        # Legacy fallback
        return [
            {"level": 1, "price": float(sig.take_profit_1), "close_percent": 0.50, "r_multiple": 1.5, "kind": "legacy"},
        ]
    if not tp_levels:
        return []

    levels = []
    entry = float(sig.entry_price) if sig.entry_price else (fill_price or 0.0)
    sl = float(sig.stop_loss) if sig.stop_loss else entry
    risk = abs(entry - sl) if entry and sl else 0.0
    for lvl in tp_levels:
        price = float(lvl["price"])
        # Re-base to fill price if provided, keeping R-multiple constant
        if fill_price is not None and risk > 0:
            r = float(lvl.get("r_multiple", 1.0))
            if sig.direction == "long":
                price = fill_price + risk * r
            else:
                price = fill_price - risk * r
        levels.append({
            "level": int(lvl.get("level", len(levels) + 1)),
            "price": price,
...
def _execute_for_bot(db, bot: BotConfig, sig: AISignal) -> dict:
    ...
    # Pre-calculate TP splits from dynamic risk plan (Fase B)
    _signal_tp_levels = _extract_signal_tp_levels(sig)
    _atomic_tp_records = [
        {
            "level": lvl["level"],
            "price": lvl["price"],
            "close_percent": int(round(lvl["close_percent"] * 100)),
            "hit": False,
            "order_id": None,
        }
        for lvl in _signal_tp_levels
    ]
    # Legacy fallback if dynamic plan is empty
    if not _atomic_tp_records and _emergency_tp1:
        _effective_status_val = _effective_status(sig)
        _tp1_pct_atomic, _tp2_pct_atomic, _tp3_pct_atomic = 40, 30, 30
    ...
    # ── 3-Stage TP Strategy from dynamic risk plan (Fase B) ──────────────────
    # Prefer confluence-aware tp_levels generated by dynamic_risk_builder.
    # They already guarantee a minimum R-ratio and adaptive close splits.
    fill_f = float(fill_price)
    signal_tp_levels = _extract_signal_tp_levels(sig, fill_price=fill_f)

    tp1_price = tp2_price = tp3_price = None
    tp1_source_final = tp2_source_final = tp3_source_final = "none"
    tp1_pct = tp2_pct = tp3_pct = Decimal("0")

    if signal_tp_levels:
        # Map dynamic levels to TP1/2/3
        lvl_map = {lvl["level"]: lvl for lvl in signal_tp_levels}
        if 1 in lvl_map:
            tp1_price = lvl_map[1]["price"]
            tp1_pct = Decimal(str(lvl_map[1]["close_percent"]))
            tp1_source_final = lvl_map[1].get("kind", "dynamic")
        if 2 in lvl_map:
            tp2_price = lvl_map[2]["price"]
            tp2_pct = Decimal(str(lvl_map[2]["close_percent"]))
            tp2_source_final = lvl_map[2].get("kind", "dynamic")
        if 3 in lvl_map:
            tp3_price = lvl_map[3]["price"]
            tp3_pct = Decimal(str(lvl_map[3]["close_percent"]))
            tp3_source_final = lvl_map[3].get("kind", "dynamic")

        logger.info(
            f"[ACTIVATOR] {bot.bot_name} dynamic TP plan from signal: "
            f"TP1={float(tp1_pct)*100:.0f}%@{tp1_price} "
            f"TP2={float(tp2_pct)*100:.0f}%@{tp2_price} "
            f"TP3={float(tp3_pct)*100:.0f}%@{tp3_price}"
        )
    else:
        # Legacy fallback: use signal.take_profit_1/2 with default 50/50 split
        tp1_source_final = "legacy_signal"
        tp2_source_final = "legacy_signal"
        tp3_source_final = "none"
        tp1_price = float(sig.take_profit_1) if sig.take_profit_1 else None
        tp2_price = float(sig.take_profit_2) if sig.take_profit_2 else None
    ...
    if total_pct > Decimal("0"):
        tp1_pct = tp1_pct / total_pct
        tp2_pct = tp2_pct / total_pct
        tp3_pct = tp3_pct / total_pct

    qty_tp1 = (quantity * tp1_pct).quantize(Decimal("0.000001")) if tp1_pct > 0 else Decimal("0")
    qty_tp2 = (quantity * tp2_pct).quantize(Decimal("0.000001")) if tp2_pct > 0 else Decimal("0")
